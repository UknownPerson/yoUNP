...
import yoUNP.api.events.world.EventMove;
...
public class Speed
extends Module {
    private Mode<Enum> mode = new Mode("Mode", "mode", (Enum[])SpeedMode.values(), (Enum)SpeedMode.Bhop);
		...
		if (this.lagcheck.getValue().booleanValue()) {
			if (ep.getPacket() instanceof S08PacketPlayerPosLook && this.deactivationDelay.delay(2000F)) {
				...
				if (Client.instance.getModuleManager().getModuleByClass(Speed.class).isEnabled()) {
					...
					//ClientUtil.sendClientMessage("(LagbackCheck)Speed Disable", ClientNotification.Type.ERROR);
				}
			}
		}
	}
